...
const RecentTaskItem = React.memo(({ task, index }: { task: ExtendedTask; index: number }) => {
  ...
  return (
    <motion.div
      ...
      className={`${isDeleted ? "opacity-60 line-through" : ""} bg-gray-50/80 backdrop-blur-sm rounded-lg border p-4 hover:shadow-md transition-shadow`}
    >
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
        <div>
          ...
          <p className="text-xs text-pink-400 mt-1">
            Created by: <span className="font-medium">{task.createdBy?.username || "Unknown"}</span>
          </p>
          <p className="text-xs text-blue-400">
            Assigned to: <span className="font-medium">{task.assignedTo?.username || "Unassigned"}</span>
          </p>
        </div>
...
const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!socket) {
      ...
      newSocket.on("newTaskAssigned", () => {
        fetchDashboardData();
      });

      newSocket.on("taskDeleted", (deletedTaskId: string) => {
        setTasks((prevTasks) =>
          prevTasks.map((task) =>
            task._id === deletedTaskId ? { ...task, deleted: true, isDeleted: true } : task
          )
        );
      });
      ...
      return () => {
        newSocket.off("newTaskAssigned");
        newSocket.off("taskDeleted");
        newSocket.disconnect();
      };
    }
  }, [user, socket, fetchDashboardData]);
    ...
    <div className="min-h-screen bg-texture bg-cover bg-fixed bg-center text-gray-800 transition-all duration-500 px-4 py-8 max-w-screen-lg mx-auto">
      {loading ? (
        ...
        <>
          ...
          {error && (
            <div
              className="bg-red-100 text-red-700 p-3 mb-4 rounded text-center"
              role="alert"
              aria-live="assertive"
            >
              {error}
          ...
          >
            ...
            <div className="space-y-4">
              {recentTasks.length > 0 ? (
                recentTasks.map((task, i) => <RecentTaskItem key={task._id} task={task} index={i} />)
              ) : (
